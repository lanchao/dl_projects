...
def build_forward_backward(H, x, googlenet, phase, boxes, flags):
    ...
    with tf.variable_scope('decoder', reuse={'train': None, 'test': True}[phase]):
        ...
        true_classes = tf.reshape(classes, [outer_size * H['rnn_len']])
        ...
        if H['use_rezoom']:
            ...
            inside_class = tf.mul(inside, tf.to_int64(tf.reshape(classes, [-1])))
            delta_confs_loss = tf.reduce_sum(
                tf.nn.sparse_softmax_cross_entropy_with_logits(new_confs, inside_class)) / outer_size * H['solver']['head_weights'][0] * 0.1
...
def train(H, test_images):
    ...
    with tf.Session(config=config) as sess:
        ...
        for phase in ['train', 'test']:
            ...
            I = tf.image.per_image_whitening(d['image'])
            image = sess.run(I)
            d['image'] = image
        ...
        start_iter = H['solver'].get('start_iter', 0)
        for i in xrange(start_iter, max_iter):
