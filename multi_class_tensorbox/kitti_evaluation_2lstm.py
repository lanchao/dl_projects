...
from train_two_lstm_two_step_training_combine import build_forward
...
def get_results(args, H):
    ...
    if H['use_rezoom']:
        pred_boxes, pred_logits, pred_confidences, pred_confs_deltas, pred_boxes_deltas, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states = build_forward(H,
                                                                                                        tf.expand_dims(
                                                                                                            x_in, 0),
                                                                                                        googlenet,
                                                                                                        'test',
        ...
        pred_boxes, pred_logits, pred_confidences, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states = build_forward(H, tf.expand_dims(x_in, 0), googlenet, 'test',
                                                                  reuse=None)
...
#    model_height = 384
    ...
    with tf.Session() as sess:
        ...
        for i in xrange(total_image):
            ...
            feed = {x_in: pad_img}
            (np_pred_boxes, np_pred_confidences) = sess.run([pred_boxes_step2, pred_confidences_step2], feed_dict=feed)
