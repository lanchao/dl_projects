    ...
    for cidx in xrange(cells_per_image):
        ...
        for bidx in xrange(min(len(box_list[cidx]), max_len)):
            ...
            if (abs(ox) < H['focus_size'] * region_size and abs(oy) < H['focus_size'] * region_size and
                ...
                rect = np.array([ox, oy, width, height], dtype=np.float)
                unsorted_boxes.append({"rect": rect, "flag": box_list[cidx][bidx].silhouetteID})

        for bidx, box in enumerate(sorted(unsorted_boxes, key=lambda x: x['rect'][0]**2 + x['rect'][1]**2)):
            boxes[0, cidx, :, bidx, 0] = box['rect']
            box_flags[0, cidx, 0, bidx, 0] = max(box['flag'], 1)
