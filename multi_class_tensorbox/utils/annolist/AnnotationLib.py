...
class Annotation(object):
    ...
    def filter(self, mapping, scale, min_size):
        new_rects = []
        for rect in self.rects:
            if rect.silhouetteID in mapping:
                rect.silhouetteID = mapping[rect.silhouetteID]
                rect.x1 = rect.x1 * scale
                rect.y1 = rect.y1 * scale
                rect.x2 = rect.x2 * scale
                rect.y2 = rect.y2 * scale
                size = min(fabs(rect.x2 - rect.x1), fabs(rect.y2 - rect.y1))
                if size >= min_size:
                    new_rects.append(rect)
        self.rects = new_rects

    def get_flip(self, img_width, img_height):

        new = Annotation()
        fn, ext = os.path.splitext(self.imageName)
        new.imageName = fn + ".flip" + ext
        new.imagePath = self.imagePath
        new.frameNr   = self.frameNr
        new.rects     = []
        for rect in self.rects:
            new_rect = copy.deepcopy(rect)
            new_rect.x1 = img_width - rect.x2
            new_rect.x2 = img_width - rect.x1 - 1
            new.rects.append(new_rect)
        return new
