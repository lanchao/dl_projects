...
def add_rectangles(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, multi_class=False):
    ...
    for n in range(rnn_len):
        for y in range(grid_height):
            for x in range(grid_width):
                ...
                class_id = get_class_id(confidences_r[0, y, x, n, 1:])
    ...
    pairs = [(all_rects_r, (255, 0, 0)), (acc_rects, (0, 255, 0))] if show_removed else [(acc_rects, (0, 255, 0))]
...
def get_class_id(confidences):
    conf = 0.
    class_id = -1
    #print 'confidences'
    #print len(confidences)
    #print confidences
    for idx in xrange(len(confidences)):
