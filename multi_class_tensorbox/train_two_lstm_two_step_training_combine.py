...
def build_lstm_inner(H, lstm_input):
    ...
    outputs = []
    with tf.variable_scope('RNN_decoder', initializer=tf.random_uniform_initializer(-0.1, 0.1)):
...
def build_lstm_inner_reuse_state(H, lstm_inputs):
    '''
    build lstm decoder
    '''
    lstm_cell = rnn_cell.BasicLSTMCell(H['lstm_size'], forget_bias=0.0, state_is_tuple=False)
    ...
    saved_states = []
    for time_step in range(H['rnn_len']):
        with tf.variable_scope('RNN_sequential_%d' % time_step, initializer=tf.random_uniform_initializer(-0.1, 0.1)):
            saved_state = tf.Variable(initial_value=tf.zeros([batch_size, lstm.state_size]), trainable=False,
                                      name='saved_state')
            state = saved_state
            input_states.append(state)
            output, state = lstm(lstm_inputs[time_step], state)
            state_save_op = saved_state.assign(state)
            outputs.append(output)
            state_save_ops.append(state_save_op)
            saved_states.append(saved_state)
    return outputs, state_save_ops, saved_states, input_states
...
def build_forward(H, x, googlenet, phase, reuse):
    ...
    with tf.variable_scope('decoder_step2', reuse=reuse, initializer=initializer):
        lstm2_inputs = []
        for lstm1_output in lstm_outputs:
            lstm2_input = tf.concat(1, [lstm_input, lstm1_output])
            lstm2_inputs.append(lstm2_input)
        lstm2_outputs, save_lstm2_state_ops, saved_states, input_states = build_lstm_inner_reuse_state(H, lstm2_inputs)

        pred_boxes_step2 = []
        pred_logits_step2 = []
        for k in range(H['rnn_len']):
            output_step2 = lstm2_outputs[k]
            if phase == 'train_step2':
                output_step2 = tf.nn.dropout(output_step2, 0.5)
            box_weights_step2 = tf.get_variable('box_ip_step2_%d' % k, shape=(H['lstm_size'], 4))
            conf_weights_step2 = tf.get_variable('conf_ip_step_2_%d' % k, shape=(H['lstm_size'], H['num_classes']))
            pred_boxes_step_step2 = tf.reshape(tf.matmul(output_step2, box_weights_step2) * 50,
                                               [outer_size, 1, 4])
            pred_boxes_step2.append(pred_boxes_step_step2)
            pred_logits_step2.append(tf.reshape(tf.matmul(output_step2, conf_weights_step2),
                                                [outer_size, 1, H['num_classes']]))
        with tf.control_dependencies(save_lstm2_state_ops):
            pred_boxes_step2 = tf.concat(1, pred_boxes_step2)
        pred_logits_step2 = tf.concat(1, pred_logits_step2)
        pred_logits_squash_step2 = tf.reshape(pred_logits_step2,
                                              [outer_size * H['rnn_len'], H['num_classes']])
        pred_confidences_squash_step2 = tf.nn.softmax(pred_logits_squash_step2)
        pred_confidences_step2 = tf.reshape(pred_confidences_squash_step2,
                                            [outer_size, H['rnn_len'], H['num_classes']])

    if H['use_rezoom']:
        return pred_boxes, pred_logits, pred_confidences, pred_confs_deltas, pred_boxes_deltas, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states
    return pred_boxes, pred_logits, pred_confidences, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states
...
def build_forward_backward(H, x, googlenet, phase, boxes, flags):
    ...
    grid_size = H['grid_width'] * H['grid_height']
    outer_size = grid_size * H['batch_size']
    reuse = {'train': None, 'test': True, 'train_step2': True, 'test_step2': True}[phase]
    if H['use_rezoom']:
        (pred_boxes, pred_logits,
         pred_confidences, pred_confs_deltas, pred_boxes_deltas, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states) = build_forward(H, x, googlenet, phase,
                                                                                               reuse)
    else:
        pred_boxes, pred_logits, pred_confidences, pred_boxes_step2, pred_logits_step2, pred_confidences_step2, saved_states, input_states = build_forward(H, x, googlenet, phase, reuse)
    with tf.variable_scope('decoder', reuse={'train': None, 'test': True, 'train_step2': True, 'test_step2': True}[phase]):
        ...
        if H['use_rezoom']:
            ...
            delta_confs_loss = tf.reduce_sum(
                tf.nn.sparse_softmax_cross_entropy_with_logits(new_confs, inside)) / outer_size * \
    ...
    with tf.variable_scope('decoder_step2',
                               reuse={'train': None, 'test': True, 'train_step2': True, 'test_step2': True}[phase]):
        # loss of step2 lstm
        pred_logit_r_step2 = tf.reshape(pred_logits_step2,
                                        [outer_size * H['rnn_len'], H['num_classes']])
        confidences_loss_step2 = (tf.reduce_sum(
            tf.nn.sparse_softmax_cross_entropy_with_logits(pred_logit_r_step2, true_classes))
                                 ) / outer_size * H['solver']['head_weights'][0]
        residual_step2 = tf.reshape(perm_truth - pred_boxes_step2 * pred_mask,
                                    [outer_size, H['rnn_len'], 4])
        boxes_loss_step2 = tf.reduce_sum(tf.abs(residual_step2)) / outer_size * H['solver']['head_weights'][1]

        loss_step2 = confidences_loss_step2 + boxes_loss_step2

    return pred_boxes, pred_confidences, loss, confidences_loss, boxes_loss, pred_boxes_step2, pred_confidences_step2, loss_step2, confidences_loss_step2, boxes_loss_step2, saved_states, input_states
...
def build(H, q):
    ...
    loss, accuracy, confidences_loss, boxes_loss, loss_step2, accuracy_step2, confidences_loss_step2, boxes_loss_step2 = {}, {}, {}, {}, {}, {}, {}, {}
    for phase in ['train', 'test', 'train_step2', 'test_step2']:
        ...
        (pred_boxes, pred_confidences,
         ...
         boxes_loss[phase], pred_boxes_step2,
         pred_confidences_step2, loss_step2[phase],
         confidences_loss_step2[phase], boxes_loss_step2[phase],
         saved_states, input_states) = build_forward_backward(H, x, encoder_net, phase, boxes, flags)
        ...
        pred_confidences_r_step2 = tf.reshape(pred_confidences_step2,
                                        [H['batch_size'], grid_size, H['rnn_len'], arch['num_classes']])
        pred_boxes_r_step2 = tf.reshape(pred_boxes_step2, [H['batch_size'], grid_size, H['rnn_len'], 4])
        ...
        b = tf.equal(tf.argmax(confidences[:, :, 0, :], 2), tf.argmax(pred_confidences_r_step2[:, :, 0, :], 2))
        accuracy_step2[phase] = tf.reduce_mean(tf.cast(b, 'float32'), name=phase + '/accuracy_step2')
        ...
        elif phase == 'train_step2':
            global_step2 = tf.Variable(0, trainable=False)
            #tvars_step2 = tf.trainable_variables()
            tvars_step2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'decoder_step2')
            if H['clip_norm'] <= 0:
                grads_step2 = tf.gradients(loss_step2['train'], tvars_step2)
            else:
                grads_step2, norm_step2 = tf.clip_by_global_norm(tf.gradients(loss_step2['train'], tvars_step2), H['clip_norm'])

            train_op_step2 = opt.apply_gradients(zip(grads_step2, tvars_step2), global_step=global_step)

        elif phase == 'test_step2':
            moving_avg = tf.train.ExponentialMovingAverage(0.95)
            smooth_op_step2 = moving_avg.apply([accuracy_step2['train_step2'], accuracy_step2['test_step2'],
                                                confidences_loss_step2['train_step2'], boxes_loss_step2['train_step2'],
                                                confidences_loss_step2['test_step2'], boxes_loss_step2['test_step2'],
                                                ])

            for p in ['train_step2', 'test_step2']:
                tf.scalar_summary('%s/accuracy_step2' % p, accuracy_step2[p])
                tf.scalar_summary('%s/accuracy_step2/smooth' % p, moving_avg.average(accuracy_step2[p]))
                tf.scalar_summary("%s/confidences_loss_step2" % p, confidences_loss_step2[p])
                tf.scalar_summary("%s/confidences_loss_step2/smooth" % p,
                                  moving_avg.average(confidences_loss_step2[p]))
                tf.scalar_summary("%s/regression_loss_step2" % p, boxes_loss_step2[p])
                tf.scalar_summary("%s/regression_loss_step2/smooth" % p,
                                  moving_avg.average(boxes_loss_step2[p]))
        ...
        if phase == 'test_step2':
            ...
            # show predictions to visualize training progress
            test_pred_confidences_step2 = pred_confidences_r_step2[0, :, :, :]
            test_pred_boxes_step2 = pred_boxes_r_step2[0, :, :, :]
            ...
            pred_log_img_step2 = tf.py_func(log_image,
                                      [test_image, test_pred_confidences_step2, test_pred_boxes_step2, global_step, 'pred_step2'],
                                      [tf.float32])
            true_log_img_step2 = tf.py_func(log_image,
                                      [test_image, test_true_confidences, test_true_boxes, global_step, 'true_step2'],
                                      [tf.float32])
            tf.image_summary(phase + '/pred_boxes_step2', tf.pack(pred_log_img_step2), max_images=10)
            tf.image_summary(phase + '/true_boxes_step2', tf.pack(true_log_img_step2), max_images=10)

    summary_op = tf.merge_all_summaries()

    return (config, loss, accuracy, summary_op, train_op,
            smooth_op, global_step, learning_rate, encoder_net, loss_step2, accuracy_step2, train_op_step2, smooth_op_step2, saved_states, input_states)
...
def train(H, test_images):
    ...
    (config, loss, accuracy, summary_op, train_op,
     smooth_op, global_step, learning_rate, encoder_net, loss_step2, accuracy_step2, train_op_step2, smooth_op_step2, saved_states, input_states) = build(H, q)
    ...
    with tf.Session(config=config) as sess:
        ...
        for phase in ['train', 'test', 'train_step2', 'test_step2']:
            ...
            gen_shuffle_data = train_utils.load_data_gen(H, phase, jitter=H['solver']['use_jitter'], shuffle_annos=True)
            gen_seq_data = train_utils.load_data_gen(H, phase, jitter=False, shuffle_annos=True)
            gen = gen_shuffle_data if (phase == 'train' or phase == 'test') else gen_seq_data
            d = gen.next()
            sess.run(enqueue_op[phase], feed_dict=make_feed(d))
        ...
        start = time.time()
        step_1_start = H['solver'].get('step1_start', 0)
        step_1_iter = H['solver'].get('step1_iter', 1000000)
        step_2_start = H['solver'].get('step2_start', 0)
        max_iter = H['solver'].get('max_iter', 10000000)
        #out = open('/tmp/train_loss_shuffle.txt', 'w')
        for i in xrange(step_1_start, step_1_iter):
         ...
         #       out.write(str(step) + ': '+str(batch_loss_train))
         #       out.write('\n')
         #       out.flush()
                # sess.run(save_lstm2_state_ops)
            else:
                ...
                # sess.run(save_lstm2_state_ops)
                #saved = saved_states[1].eval()
                #print 'saved states 0:'
                #print saved
                ...
                # print 'lstm states'
                # for update in save_lstm2_state_ops:
                #    print update.eval()

            if global_step.eval() % H['logging']['save_iter'] == 0 or global_step.eval() == max_iter - 1:
                saver.save(sess, ckpt_file, global_step=global_step)

        for j in xrange(step_2_start, max_iter):
            display_iter = H['logging']['display_iter']
            adjusted_lr = (H['solver']['learning_rate'] *
                           0.5 ** max(0, (j / H['solver']['learning_rate_step']) - 2))
            lr_feed = {learning_rate: adjusted_lr}

            if j % display_iter != 0:
                # train network
                batch_loss_train, _ = sess.run([loss_step2['train_step2'], train_op_step2], feed_dict=lr_feed)
         #       step = global_step.eval()
         #       out.write(str(step) + ': '+str(batch_loss_train))
         #       out.write('\n')
         #       out.flush()
                ...
                #print inputs
                (train_loss, test_accuracy, summary_str,
                 _, _) = sess.run([loss_step2['train_step2'], accuracy_step2['test_step2'],
                                   summary_op, train_op_step2, smooth_op_step2,
                                   ], feed_dict=lr_feed)
                # sess.run(save_lstm2_state_ops)
                #saved = saved_states[1].eval()
                #print 'saved states 0:'
                ...
                print(print_str %
                      (j, adjusted_lr, train_loss,
                       test_accuracy * 100, dt * 1000 if j > 0 else 0))
